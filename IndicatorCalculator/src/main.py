...
port = 5670
agent_name = "IndicatorCalculator"
device = None
verbose = False
...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, IndicatorCalculator)
...
def indicators_param_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, IndicatorCalculator)
        agent_object.indicators_paramI = value
        # add code here if needed
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    agent = IndicatorCalculator()

    igs.observe_agent_events(on_agent_event_callback, agent)
    igs.observe_freeze(on_freeze_callback, agent)

    igs.input_create("historical_data", igs.DATA_T, None)
    igs.input_create("indicators_param", igs.DATA_T, None)

    igs.output_create("indicators", igs.DATA_T, None)

    igs.observe_input("historical_data", historical_data_input_callback, agent)
    igs.observe_input("indicators_param", indicators_param_input_callback, agent)
