...
from ChartDisplay import *

port = 5670
agent_name = "ChartDisplay"
device = None
verbose = False
...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, ChartDisplay)
...
# inputs
def indicators_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, ChartDisplay)
        agent_object.indicatorsI = value
        # add code here if needed
    except:
        print(traceback.format_exc())

def historical_data_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, ChartDisplay)
        agent_object.historical_dataI = value
        # add code here if needed
    except:
        print(traceback.format_exc())

def order_result_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, ChartDisplay)
        agent_object.order_resultI = value
        # add code here if needed
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.input_create("indicators", igs.DATA_T, None)
    igs.input_create("historical_data", igs.DATA_T, None)
    igs.input_create("order_result", igs.DATA_T, None)

    igs.observe_input("indicators", indicators_input_callback, agent)
    igs.observe_input("historical_data", historical_data_input_callback, agent)
    igs.observe_input("order_result", order_result_input_callback, agent)
