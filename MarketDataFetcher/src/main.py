...
from MarketDataFetcher import *

port = 5670
agent_name = "MarketDataFetcher"
device = None
verbose = False
...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, MarketDataFetcher)
...
# inputs
def symbol_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, MarketDataFetcher)
        agent_object.symbolI = value
        # add code here if needed
    except:
        print(traceback.format_exc())

def timeframe_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, MarketDataFetcher)
        agent_object.timeframeI = value
        # add code here if needed
    except:
        print(traceback.format_exc())
...
def fetch_request_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, MarketDataFetcher)
        agent_object.fetch_requestI = value
        # add code here if needed
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.input_create("symbol", igs.STRING_T, None)
    igs.input_create("timeframe", igs.STRING_T, None)
    igs.input_create("start_time", igs.DOUBLE_T, None)
    igs.input_create("end_time", igs.DOUBLE_T, None)
    igs.input_create("fetch_request", igs.DATA_T, None)

    igs.output_create("historical_data", igs.DATA_T, None)

    igs.observe_input("symbol", symbol_input_callback, agent)
    igs.observe_input("timeframe", timeframe_input_callback, agent)
    igs.observe_input("start_time", start_time_input_callback, agent)
    igs.observe_input("end_time", end_time_input_callback, agent)
    igs.observe_input("fetch_request", fetch_request_input_callback, agent)
